...
class Agent {
  ...
  update(dt) {
	...
	for(let i = 0; i < obs.length; i++){
		...
		//console.log( i + '....' + obs[i].center.distanceTo (this.pos));
		if(minProj > proj && proj > 0 && obs[i].center.distanceTo (this.pos) < 150){
			minProj = proj;
			count = i;
			var vhatMin = this.vel.clone().normalize();
			var pointMin = obs[i].center.clone().sub (this.pos) // c-p
		}
	}
		
		//const REACH = 200
		const K = 10
		console.log(count + '...' + minProj);
		
		if (minProj > 0 && minProj < REACH) {
			//console.log(count);
			let perp = new THREE.Vector3();
			perp.subVectors (pointMin, vhatMin.clone().setLength(minProj));
			let overlap = obs[count].size + this.halfSize - perp.length();
			//console.log(perp.length());
			if (overlap >  0) {
